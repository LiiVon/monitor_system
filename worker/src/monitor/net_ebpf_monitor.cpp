    ...
    if (bpf_map_lookup_elem(map_fd_, &next_key, &stats) == 0)
    {
      ...
      if (cache_it != cache_.end())
      {
        ...
        if (old_duration > 0)
        {
          ...
          // 处理计数器溢出
          if (rcv_diff < 0)
            rcv_diff = stats.rcv_bytes;
          if (snd_diff < 0)
            snd_diff = stats.snd_bytes;
          if (rcv_pkt_diff < 0)
            rcv_pkt_diff = stats.rcv_packets;
          if (snd_pkt_diff < 0)
            snd_pkt_diff = stats.snd_packets;
      ...
      // 更新缓存
      cache_[next_key] = {stats.rcv_bytes, stats.rcv_packets, stats.snd_bytes,
                          stats.snd_packets, now};
    }
    key = next_key;
  }
