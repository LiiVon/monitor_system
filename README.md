...
```
# 安装编译依赖（Ubuntu 22.04 使用 LLVM 12）
sudo apt install -y bison build-essential cmake flex git libedit-dev \
    libllvm12 llvm-12-dev libclang-12-dev python3 zlib1g-dev \
    libelf-dev libfl-dev liblzma-dev python3-distutils \
    linux-headers-$(uname -r)

# 编译安装 BCC（放到独立目录，不要和项目混在一起）
cd ~
git clone https://github.com/iovisor/bcc.git bcc-build
cd bcc-build

# 如果 cmake 报 "Unable to find clang libraries"，
# 先补装 sudo apt install -y libclang-14-dev，然后重新 cmake
...
# 安装完成后，BCC 工具在 /usr/share/bcc/tools/ 下
ls /usr/share/bcc/tools

```

### libbpf 库路径修复
手动编译的 libbpf 装在 /usr/lib64/，Ubuntu 默认不搜这个路径，会导致 sudo 执行时报 libbpf.so.1: cannot open shared object file。

``` 
# 创建软链接让系统能搜到
sudo ln -sf /usr/lib64/libbpf.so.1 /usr/lib/x86_64-linux-gnu/libbpf.so.1
sudo ldconfig

# 验证
ldconfig -p | grep libbpf
# 应看到 libbpf.so.1 和 libbpf.so.0 两条记录
...
### 4.验证
```
mysql -u monitor -pmonitor123 -e "USE monitor_db; SHOW TABLES;"
...
```bash
# 克隆项目
git clone https://github.com/LiiVon/monitor_system.git
cd monitor_system
...
验证数据库
```
mysql -u monitor -pmonitor123 -e "USE monitor_db; SELECT COUNT(*) FROM server_performance;"
